...
for mag, co in zip(discovery.keys(), ['b', 'c', 'g', 'orange', 'r',]) :
    if mag == 99.:
        label = 'Bright limit'
    else:
        label = 'Magnitude %.1f'%mag[3:]
    ax1.plot(range(3,9), np.array(discovery[mag])/fleet_size, label=label, 
             color=co, linewidth=3)
plt.ylim(0,1)
...
plt.grid()
plt.legend()
plt.savefig('detection_histogramTEST.png', dpi=100)
#plt.show()
plt.close('all')
