'''
This program should be run once to create 10 files, each of which has 1/10 of the total 
sphere points specifically in the footprint.  Something might have failed on this and not 
covered the whole footprint.  Each one of these can be used but I only used one.
'''
...
def footprint(ra, dec):
    # determine if position is in the footprint 
...
while Ncount < N:
    if Ncount < N:
        ...
        for m in range(int(M_theta)):
            print m, '/', int(M_theta) # hard to see how long it'll take otherwise
...
for i, s in enumerate([s0, s1, s2, s3, s4, s5, s6, s7, s8, s9]):
    # write and plot each set of the points
    print i
